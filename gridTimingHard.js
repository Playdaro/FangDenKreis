// gridTimingHard.js – Ampel-Variante mit 10.000 ms Gesamtzeitbank (nur bei Grün)
// Hard: max. Warten 10 s, sehr kurzes Klickfenster (350 ms), Feldwechsel nach JEDEM Treffer.
...
export function startGridTimingHard() {
  ...
  const maxWaitSec      = 10;     // spätestes Umschalten zu Grün
  const greenWindowMs   = 500;    // sehr kurzes Klickfenster
  const changeAfterHits = 1;      // nach jedem Treffer anderes Feld
  const difficulty = 'hard';
  const modeId     = 'grid-timing-hard';
  ...
  async function runRound() {
    ...
    ticker = setInterval(() => {
      ...
      if (!isGreen && now >= plannedGreenTs) {
        ...
        // sehr kurzes Klickfenster (verpassen => Miss)
  ...
  function endRound(hit, clickTime) {
    ...
    clearCells();
    if (remainingMs > 0) {
      setTimeout(() => runRound(), 300); // kurze Verschnaufpause; Budget pausiert (nicht grün)
    }
  }
