// gridTimingEasy.js – Ampel-Variante mit 1.000 ms Gesamtzeitbank
// Die 1s laufen NUR, während die Zielzelle grün ist. Nach Klick pausiert der Timer,
...
export function startGridTimingEasy() {
  // --- Easy-Parameter ---
  const sessionBudgetMs = 10_000; // 10.000 ms = 10 s
  const maxWaitSec      = 5;     // spätestes Umschalten zu Grün
  const greenWindowMs   = 700;   // kurzes Klickfenster
  const changeAfterHits = 3;     // nach 3 Treffern neues Feld
  const difficulty = 'easy';
  const modeId     = 'grid-timing-easy';
  ...
  function finish(reason = 'ended') {
    ...
    document.dispatchEvent(new CustomEvent('gridtiming:finished', {
      detail: {
        ...
        misses,
        bestStreak,
        duration: sessionBudgetMs / 1000, // 1.000 ms Budget (nur Grün-Zeit)
        difficulty,
        modeId,
        avgError
      }
