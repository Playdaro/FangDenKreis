// gridCore.js – Master HUD + Master Playfield (3×3)
// Logik bleibt wie vorher, nur UI wird vereinheitlicht.
...
function getResultScreen() {
  return document.getElementById('screen-grid-result');
}
...
function ensureGridUI() {
  ...
  if (backBtn) {
    backBtn.onclick = () => stopGrid('abort', false);
  }
...
function updateHud() {
  ...
  if (missEl)   missEl.textContent   = String(state.misses);
...
function spawnNewTarget(excludeIdx = null) {
  ...
  const candidates = (state.allowedIndices && state.allowedIndices.length)
...
function registerReactionHit() {
  ...
  const rtSec = (now - state.lastSpawnTime) / 1000;
...
function onCellClick(e) {
  ...
  if (isHit) {
    ...
    state.spawnMs = Math.max(state.minSpawnMs, Math.round(state.spawnMs * state.speedDecay));
...
function tick() {
  ...
  if (state.currentIndex != null && state.spawnMs > 0) {
    if (now - state.lastSpawnTime >= state.spawnMs) {
      // Ziel ausgelaufen → Miss + neues Ziel
      state.misses++;
      state.streak = 0;
...
function finishGrid(endReason = 'timeup', record = true) {
  ...
  if (res) {
    ...
    set('grid-res-diff', state.label || state.diffKey || '–');
    set('grid-res-hits', String(hits));
    set('grid-res-misses', String(misses));
    set('grid-res-avg', Math.round(avgRt * 1000) + ' ms');

    let reasonText = '–';
    switch (endReason) {
      case 'timeup':  reasonText = 'Zeit abgelaufen'; break;
      case 'abort':   reasonText = 'Abgebrochen';     break;
      default:        reasonText = String(endReason); break;
    }
    set('grid-res-endreason', reasonText);
